...
describe('InventoryPage', () => {
  ...
  it('shows the placeholder list with the count when items exist', async () => {
    makeSupabaseMock({
      crew_members: {
        maybeSingle: { data: { crew_id: 'crew_abc' }, error: null },
      },
      inventory_items: { select: { count: 3, error: null } },
    })
    renderWithRouter(<InventoryPage />)
    await waitFor(() => {
      expect(screen.getByText(/3 items in your inventory/i)).toBeInTheDocument()
    })
