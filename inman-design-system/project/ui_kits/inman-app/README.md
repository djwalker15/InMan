...
A hi-fi recreation of the InMan mobile app, modeled on the Figma `Screens/` page.

## Files
- `index.html` — interactive click-thru: Landing → Sign Up → Crew Decision → Crew Naming → Spaces Intro → Dashboard.
- `Brand.jsx` — wordmark + glyph, nav header (`Type=SignUp` / `Type=Close` / `Type=SignedIn` variants).
...
## Design width
390px (canonical mobile) framed inside an iOS device bezel.
